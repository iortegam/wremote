...
class Controller(object):
    ...
    def __init__(self, ctlFvars):
        ...
        #self.ser      = serial.Serial(ctlFvars['ATDSIS_PORT'], ctlFvars['ATDSIS_RATE'])

        try:
            self.ser = serial.Serial(ctlFvars['ATDSIS_PORT'], ctlFvars['ATDSIS_RATE'], timeout = 2)
        except IOError as e:
            print("%s - exiting..." % (e))
            exit()
    ...
    def readRegister(self, address, node = b'\x01', subindex = 0, debug = False):
        ...
        #print(struct.unpack('<f',  self.GET + int.to_bytes(address, 2, 'little') + int.to_bytes(subindex, 1, 'little')))
        if debug: 
            print(dump(command))
    ...
    #--------------------------------------------
    # Read Analog Inputs: https://www.faulhaber.com/fileadmin/Import/Media/EN_7000_05048.pdf  (Page 83)
    #--------------------------------------------       
    def ReadAnalogIn(self, address=0x2313, subindex=0x04, node = b'\x01'):

        #value = hex(int.from_bytes(self.readRegister(address, node=node, subindex = subindex), byteorder='little'))
        #print(self.readRegister(address, node=node, subindex = subindex))
        value = int.from_bytes(self.readRegister(address, node=node, subindex = subindex), byteorder='little', signed=True)
        #value = self.readRegister(address, node=node, subindex = subindex)
...
class Motor(Controller):
    ...
    def getPositionLim(self, subindex=1):
        answer = self.controller.readRegister(0x607b, node = self.node, debug = False, subindex=subindex)
        position = int.from_bytes(answer, byteorder='little', signed = True)
        return position

    def getGearRatio(self, subindex=1):
        answer = self.controller.readRegister(0x6091, node = self.node, debug = False, subindex=subindex)
        position = int.from_bytes(answer, byteorder='little', signed = True)
        return position

    def getNominalSpeed(self):
        answer = self.controller.readRegister(0x6081, node = self.node, debug = False)
        position = int.from_bytes(answer, byteorder='little', signed = True)
        return position
    ...
    def setNominalSpeed(self, value=1000):
        self.controller.setRegister(0x6081, value, 4, node = self.node)
    ...
    def setPositionLimit(self, value, subindex=1):
        self.controller.setRegister(0x607b, value, 4, node = self.node, subindex = subindex)

    def setGearRatio(self, value, subindex=1):
        self.controller.setRegister(0x6091, value, 4, node = self.node, subindex = subindex)
    ...
    def enable_1(self):   #simple enable of axis
        self.shutDown()
        self.switchOn()
        self.enable()
        print ('Enable axis '+str(self)+' successful')
